...
class what:#																	||
	...
	def get(self, thing=None):#											||
		...
		self.text = str(text)
		try:#															||
			doct = text.replace('\t','  ')#								||correct yaml format
			self.dikt = yload(doct, Loader=Loader)#						||load yaml str to dikt
			del self.text
		except Exception as e:#											||
...
#			last = self.now + dt.timedelta(days=(pos - self.now.weekday()), time=(gtime-self.now.time()))#	||
		...
		return day
	def uutc(self):#													||
#		ts = datetime.datetime.fromtimestamp((u.time - 0x01b21dd213814000L)*100/1e9)
		print(ts)
		return self#													||
	def _type(self):#													||
...
class who:#																||
	...
	def prime(self, prime='LEXI'):#										||
		...
		self.tmplts = ['tmplts']
		asprime = what().get(asprime).dikt['prime']#					||
		prime = '/home/solubrew/ActiveProjects/devLEXI/3_Work/1_DELTA/'
